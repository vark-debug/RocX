...
const getTheme = (hostName: string) => {
  ...
  if (hostName === "indesign") {
    const INDESIGN_THEME_TABLE: Record<number, string> = {
      1: "lightest",
      0.51: "light",
      0.5: "dark",
      0: "darkest",
    ...
    if (overrideTheme) theme = overrideTheme as any;
  }
  return theme;
};
...
export const getColorScheme = async () => {
  ...
  let colors: any = colorTable[theme];
  ...
  return { theme, colors: colors as any };
};
